...
//         return res.status(400).json({success: false, message: "Password must be alleast 8characters"});
//     }
...
//         res.status(201).json({ success: true, token, user: { id: user._id, name, email: user.email } });
//     } 
...
export async function updatePassword(req, res) {
    ...
    if (!currentPassword || !newPassword || newPassword.length < 8) {
        return res.status(400).json({ success: false, message: "Password invalid or too short" }); // ✅ fixed
    }
